...
Detector::Detector(const std::string & model_path)
{
    ...
    CUDA_CHECK(cudaMalloc((void **)&buffers[inputIndex], 3 * INPUT_H * INPUT_W * sizeof(float)));
    CUDA_CHECK(cudaMalloc((void **)&buffers[outputIndex], output_size * sizeof(float)));
...
bool Detector::detect(Detection_pack & detection_pack)
{
    ...
    // 把图像数据拷贝至GPU, 使用`cudaMemcpyAsync` 比 `memcpy` 快10ms左右
    CUDA_CHECK(cudaMemcpyAsync(
        buffers[0] + 0 * INPUT_W * INPUT_H, pre_split[0].data, INPUT_W * INPUT_H * sizeof(float),
        cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(
        buffers[0] + 1 * INPUT_W * INPUT_H, pre_split[1].data, INPUT_W * INPUT_H * sizeof(float),
        cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(
        buffers[0] + 2 * INPUT_W * INPUT_H, pre_split[2].data, INPUT_W * INPUT_H * sizeof(float),
        cudaMemcpyHostToDevice, stream));
    ...
    // 将推理结果拷贝出来
    CUDA_CHECK(cudaMemcpyAsync(
        prob, buffers[1], output_size * sizeof(float), cudaMemcpyDeviceToHost, stream));
